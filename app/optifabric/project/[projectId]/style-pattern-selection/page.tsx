...
import {
  calculateStylePatternWorkspaceStatistics,
  groupStylePatterns,
  StylePatternGroup,
  StylePatternGroupId,
} from "@/lib/optifabric/stylePatternGrouping";
...
const [openGroups, setOpenGroups] =
  useState<Set<StylePatternGroupId>>(
    new Set<StylePatternGroupId>([
      "main-fabric",
      "custom",
    ])
  );
  ...
  const groupedPatterns = useMemo<
    StylePatternGroup<StylePatternStatus>[]
  >(() => {
    return groupStylePatterns(
      filteredPatterns,
      {
  ...
  const workspaceStatistics = useMemo(() => {
    if (!project) {
      return calculateStylePatternWorkspaceStatistics(
        []
      );
    }
  ...
  console.log(
    "OptiFabric grouped patterns:",
    groupedPatterns
  );

  console.log(
    "OptiFabric workspace statistics:",
    workspaceStatistics
  );
  const filtersActive =
    ...
    <main className="min-h-screen bg-slate-950 px-5 py-8 text-white sm:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-8 rounded-3xl border border-violet-400/20 bg-violet-950/10 p-5 sm:p-7">
          ...
          <div className="mb-6 flex flex-col gap-4 rounded-3xl border border-slate-700 bg-slate-900 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.25em] text-violet-300">
                Engineering Pattern Groups
              </p>

              <h2 className="mt-2 text-3xl font-black">
                Material-Based Pattern Workspace
              </h2>

              <p className="mt-3 max-w-4xl leading-7 text-slate-400">
                Pattern pieces are organised by material and
                engineering purpose. Open only the section
                currently being reviewed by the cutting master.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                ...
                className="rounded-xl border border-violet-400/30 bg-violet-950/30 px-5 py-3 font-black text-violet-300 transition hover:bg-violet-900/40"
              >
                Open All Groups
              </button>
          ...
          {groupedPatterns.length > 0 ? (
            <div className="space-y-6">
              {groupedPatterns.map((group) => {
                ...
                return (
                  <section
                    key={definition.id}
                    className={`overflow-hidden rounded-3xl border ${definition.theme.borderClass} ${definition.theme.backgroundClass}`}
                  >
                    <button
                      type="button"
                      onClick={() =>
                      ...
                      className={`w-full px-5 py-5 text-left transition hover:brightness-110 sm:px-7 ${definition.theme.headerBackgroundClass}`}
                    ...
                    >
                      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <div className="flex flex-wrap items-center gap-3">
                              <h3
                                className={`text-2xl font-black ${definition.theme.titleClass}`}
                              >
                                {definition.label}
                              </h3>

                              <span
                                className={`rounded-full border px-3 py-1 text-xs font-black uppercase tracking-wider ${definition.theme.badgeClass}`}
                              >
                                {
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          <GroupMetric
                            label="Uploaded"
                            value={`${statistics.uploadedPatterns}/${statistics.totalPatterns}`}
                          />

                          <GroupMetric
                            label="Included"
                            value={String(
                              statistics.includedPatterns
                            )}
                          />
                          ...
                          <GroupMetric
                            label="Cut Qty"
                            value={String(
                              statistics.totalCutQuantity
                            )}
                          />

                          <div
                            className={`flex h-11 w-11 items-center justify-center rounded-xl border font-black ${definition.theme.badgeClass}`}
                          >
                            {groupOpen ? "−" : "+"}
                          </div>
                        </div>
                    ...
                    {groupOpen ? (
                      <div className="border-t border-white/10 p-5 sm:p-7">
                        <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                          <GroupStatusCard
                            label="Validated"
                            value={`${statistics.validatedPatterns}/${statistics.totalPatterns}`}
                            passed={
                              statistics.validatedPatterns ===
                              statistics.totalPatterns
                            }
                          />

                          <GroupStatusCard
                            label="Ready for Marker"
                            value={`${statistics.readyForMarkerPatterns}/${statistics.markerPatterns}`}
                            passed={
                              statistics.markerPatterns >
                                0 &&
                              statistics.readyForMarkerPatterns ===
                                statistics.markerPatterns
                            }
                          />
                          ...
                          <GroupStatusCard
                            ...
                            value={String(
                              statistics.missingUploadPatterns
                            )}
                            passed={
                              statistics.missingUploadPatterns ===
                              0
                          ...
                          <GroupStatusCard
                            ...
                            value={String(
                              statistics.invalidQuantityPatterns
                            )}
                            passed={
                              statistics.invalidQuantityPatterns ===
                              0
            ...
            <div className="rounded-3xl border border-dashed border-violet-400/30 bg-violet-950/10 px-6 py-12 text-center">
              ...
              <p className="mt-3 text-slate-400">
                Clear the filters to display the complete
                engineering pattern workspace.
              </p>
...
function GroupMetric({
  ...
  return (
    <div className="min-w-20 rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2 text-center">
