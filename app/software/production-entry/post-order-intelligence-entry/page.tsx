...
const content = {
  en: {
    ...
    saving: "Saving...",
    success: "Post order recovery intelligence saved successfully.",
    error:
  ...
  bn: {
    ...
    eyebrow: "MBNCON পোস্ট অর্ডার রিকভারি ইন্টেলিজেন্স",
    subtitle:
    ...
    success: "পোস্ট অর্ডার রিকভারি সফলভাবে সংরক্ষিত হয়েছে।",
    error:
...
export default function PostOrderIntelligenceEntryPage() {
  ...
  const productionGap = useMemo(() => {
    return toNumber(form.orderedQty) - toNumber(form.shippedQty);
  }, [form.orderedQty, form.shippedQty]);

  const recoveryRiskLevel = useMemo(() => {
    const ageing = toNumber(form.inventoryAgeingDays);
    const rejected = toNumber(form.rejectedQty);
    const surplus = toNumber(form.surplusQty);

    if (ageing > 180 || rejected > 500 || surplus > 2000) {
      return "Critical";
    }

    if (ageing > 90 || rejected > 200 || surplus > 1000) {
      return "High";
    }
  ...
  const estimatedRecoveryRecommendation = useMemo(() => {
    const ageing = toNumber(form.inventoryAgeingDays);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addPostOrderIntelligenceEntry({
        ...
        orderedQty: toNumber(form.orderedQty),
        shippedQty: toNumber(form.shippedQty),

        surplusQty: toNumber(form.surplusQty),
        shortShipmentQty: toNumber(form.shortShipmentQty),

        rejectedQty: toNumber(form.rejectedQty),
        secondsQty: toNumber(form.secondsQty),

        reworkQty: toNumber(form.reworkQty),
        reworkCost: toNumber(form.reworkCost),
        reworkHours: toNumber(form.reworkHours),

        inventoryAgeingDays: toNumber(
          form.inventoryAgeingDays
        ),

        recoveryValue: toNumber(form.recoveryValue),
        ...
        >
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <NumberInput
              ...
              onChange={(v) => updateField("orderedQty", v)}
            />
            ...
            <NumberInput
              ...
              onChange={(v) => updateField("shippedQty", v)}
            />
            ...
            <NumberInput
              ...
              onChange={(v) => updateField("surplusQty", v)}
            />
            ...
            <NumberInput
              ...
              onChange={(v) =>
                updateField("shortShipmentQty", v)
              }
            />
            ...
            <NumberInput
              ...
              onChange={(v) => updateField("rejectedQty", v)}
            />
            ...
            <NumberInput
              ...
              onChange={(v) => updateField("secondsQty", v)}
            />
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <NumberInput
              ...
              onChange={(v) => updateField("reworkQty", v)}
            />
            ...
            <NumberInput
              ...
              onChange={(v) => updateField("reworkCost", v)}
            />
            ...
            <NumberInput
              ...
              onChange={(v) => updateField("reworkHours", v)}
            />
            ...
            <NumberInput
              ...
              onChange={(v) =>
                updateField("inventoryAgeingDays", v)
              }
            />
            ...
            <NumberInput
              ...
              onChange={(v) => updateField("recoveryValue", v)}
            />
            ...
            <SelectInput
              ...
              onChange={(v) =>
                updateField("cancellationRisk", v)
              }
            />
          ...
          <div className="mt-8 flex items-center gap-4">
            <button
              type="submit"
              disabled={saving}
              className="rounded-2xl bg-amber-700 px-6 py-3 font-semibold text-white hover:bg-amber-800 disabled:opacity-60"
            >
