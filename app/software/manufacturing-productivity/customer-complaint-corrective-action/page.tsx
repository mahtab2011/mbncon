...
export default function CustomerComplaintCorrectiveActionPage() {
  ...
  const summary = useMemo(() => {
    ...
    const totalAffected = rows.reduce((sum, r) => sum + r.quantityAffected, 0);

    return { open, critical, totalAffected };
  }, [rows]);
      ...
      <section className="bg-linear-to-r from-red-950 via-violet-900 to-blue-950 px-6 py-16 text-white">
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="max-w-6xl text-5xl font-extrabold leading-tight">
            Turn Customer Complaints Into Root-Cause Learning and Corrective Action
          </h1>

          <p className="mt-6 max-w-5xl text-xl leading-relaxed text-white/85">
            This TQM module captures customer complaints, severity, affected
            quantity, root causes, corrective actions, responsible owners,
            status, and follow-up so quality problems become structured
              ...
              <div className="mt-8 space-y-5">
                {[
                  ...
                  <label key={key} className="block">
                    ...
                    <input
                      value={String(form[key as keyof ComplaintRow])}
                      onChange={(e) =>
                        setForm({
                          ...form,
                          [key]: e.target.value,
                ...
                <label className="block">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        severity: e.target.value as ComplaintRow["severity"],
                      })
                    }
                    className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 outline-none focus:border-red-600"
                  >
                ...
                <label className="block">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        status: e.target.value as ComplaintRow["status"],
                      })
                    }
                    className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 outline-none focus:border-red-600"
                  >
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-slate-200 text-left">
                      <th className="rounded-l-xl p-4">Customer</th>
                  ...
                  <tbody>
                    {rows.map((row, index) => (
                      <tr key={index} className="border-b border-slate-200">
                        ...
                        <td className="p-4">{row.complaintType}</td>
                        <td className="p-4">{row.severity}</td>
                        <td className="p-4">{row.quantityAffected}</td>
                ...
                {rows.map((row, index) => (
                  ...
                  >
                    ...
                    <p className="mt-3 text-lg text-slate-700">
                      <strong>Root Cause:</strong> {row.rootCause}
                    </p>
              ...
              {[
                ...
                <div key={item} className="rounded-2xl bg-white/10 p-6">
                  ...
                  <p className="mt-4 text-slate-300">
                    Standard TQM implementation step to ensure complaints are
                    managed through evidence, accountability, and follow-up.
                  </p>
                </div>
              ))}
            </div>
