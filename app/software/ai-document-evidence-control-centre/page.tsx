  ...
  useEffect(() => {
    ...
    async function loadEvidenceData() {
      ...
      try {
        ...
        setRiskReports(results[0].status === "fulfilled" && Array.isArray(results[0].value) ? results[0].value : []);
        setMaintenanceLogs(results[1].status === "fulfilled" && Array.isArray(results[1].value) ? results[1].value : []);
        ...
        setProductionLogs(results[4].status === "fulfilled" && Array.isArray(results[4].value) ? results[4].value : []);
      } catch (err) {
    ...
    return [
      {
        ...
        status: riskCount > 5 ? "Review Needed" : riskCount > 0 ? "Controlled" : "Missing Evidence",
        reason: riskCount > 0 ? `${riskCount} risk record(s) found for management review.` : "No risk evidence found for current factory review.",
        action: "Ensure each risk has owner, date, root cause, corrective action and closure evidence.",
      },
      {
        title: "Maintenance Breakdown Evidence",
        category: "Maintenance",
        owner: "Maintenance Manager",
        status: breakdownHours > 20 ? "Escalate" : maintenanceCount > 0 ? "Controlled" : "Missing Evidence",
        reason: maintenanceCount > 0 ? `${maintenanceCount} maintenance record(s), ${breakdownHours} downtime hour(s).` : "No maintenance evidence found.",
        action: "Attach breakdown reason, repair note, spare usage and preventive action proof.",
      },
      {
        title: "Wastage & Rejection Evidence",
        category: "Quality / Production",
        owner: "Quality Manager / Production Head",
        status: wastageCost > 15000 ? "Escalate" : wastageCount > 0 ? "Review Needed" : "Missing Evidence",
        reason: wastageCount > 0 ? `${wastageCount} wastage record(s), estimated loss ${wastageCost}.` : "No wastage evidence found.",
        action: "Keep photos, inspection notes, defect reason, department owner and recovery action.",
      },
      {
        title: "Shipment Delay Evidence",
        category: "Export",
        owner: "Export / Merchandising Team",
        status: exportDelay > 12 ? "Escalate" : exportCount > 0 ? "Controlled" : "Missing Evidence",
        reason: exportCount > 0 ? `${exportCount} export record(s), ${exportDelay} delay day(s).` : "No export delay evidence found.",
        action: "Attach buyer communication, shipment plan, revised date and delay approval note.",
      },
      {
        title: "Production Output Evidence",
        category: "Production",
        owner: "Production Manager",
        status: productionValue > 0 ? "Controlled" : productionCount > 0 ? "Review Needed" : "Missing Evidence",
        reason: productionCount > 0 ? `${productionCount} production record(s), output value ${productionValue}.` : "No production output evidence found.",
        action: "Maintain daily output proof, line report, supervisor sign-off and exception notes.",
      },
    ];
  }, [riskReports, maintenanceLogs, wastageLogs, exportLogs, productionLogs]);

  const controlledCount = evidenceItems.filter((item) => item.status === "Controlled").length;
  const evidenceScore = Math.round((controlledCount / evidenceItems.length) * 100);
      ...
      <main className="min-h-screen bg-slate-950 text-white">
        <section className="border-b border-white/10 bg-[linear(#0f172a,#020617)] px-6 py-8">
          <div className="mx-auto max-w-7xl">
            ...
            {error ? <p className="mt-5 text-sm text-yellow-300">{error}</p> : null}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-6 pb-10">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
            <h2 className="text-2xl font-bold">Evidence details</h2>
        ...
        <section className="mx-auto max-w-7xl px-6 pb-10">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
            <h2 className="text-2xl font-bold">Evidence control checklist</h2>

            <div className="mt-6 grid gap-4 md:grid-cols-3">
              {checklistItems.map((item, index) => {
                const id = `checklist-${slugify(item)}`;
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-3 text-sm leading-7 text-slate-300">
                      This checklist point supports audit readiness,
                      accountability, traceability, and evidence-based
                      management review before internal or external inspection.
                    </p>
                  </section>
                );
              })}
